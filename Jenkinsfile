...
sh 'mvn -f pom.xml -s $settings clean install -DskipTests'
   ...
   stage('Governance Ruleset') {
      		steps {
				...
				sh 'anypoint-cli --username=jilty --password=Jilty@123 --organization=b0beec90-c6cc-4c36-8f7c-ef1f8f91253b governance api validate --rulesets ruleset.yaml  jenkins-test-12.zip'
        	            }
      		}
	} 
            ...
            steps {
                script {
                    timeout(time: 1, unit: 'HOURS') { 
                        sh "curl -u admin:sonarqubenjclabs123 -X GET -H 'Accept: application/json' http://142.93.218.194:9000/api/qualitygates/project_status?projectKey=com.mycompany:jenkins-test-12 > status.json"
...
cucumber(failedFeaturesNumber: -1, failedScenariosNumber: -1, failedStepsNumber: -1, fileIncludePattern: 'cucumber.json', jsonReportDirectory: 'target', pendingStepsNumber: -1, skippedStepsNumber: -1, sortingMethod: 'ALPHABETICAL', undefinedStepsNumber: -1)
  ...
  stage('Deploy application to cloudHub'){
   steps{
    configFileProvider([configFile(fileId: 'da01fc76-5c2b-4f0d-948a-c101b4cc4340', variable: 'settings')]){
  sh 'mvn -f pom.xml -s $settings deploy -DmuleDeploy -DskipTests -Dusername=jilty -Dpassword=Jilty@123 -DapplicationName=jenkins-test-12 -Dap.client_id=fda777bd3e3b4fcb93aff995fea2043d  -Dap.client_secret=4193AA1986054C548Bf757fd1B7F6f18 -Dapp.runtime.server=4.4.0 -Ddeployment.env=dev  -Dsecure.key=mule -Dworkers=1 -DworkerType=null -Dapi.id=18332555  -Danypoint.businessGroup="NJC POC"'
